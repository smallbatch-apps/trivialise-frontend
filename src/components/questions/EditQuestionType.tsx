...
const EditQuestionType: FC<Props> = ({ question }) => {
  ...
  return (
    <div className="flex items-center w-full my-3">
      ...
      {!editing && (
        <div className="p-2 bg-white rounded-sm border border-white w-full hover:border-gray-300 cursor-pointer" onClick={() => setEditing(true)}>
          <span className="inline-block mr-5">{QuestionTypeLabels[question.type]}</span>
        </div>
      )}
      {editing && (
        <>
          <select name="type" className="p-2 border rounded-sm flex-grow text-sm" defaultValue={question.type}>
