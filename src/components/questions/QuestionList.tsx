...
const allQuestionTypes = Object.entries(QuestionTypeLabels).map(([value, label]) => ({ value: +value, label }));

const QuestionList: FC<Props> = ({ tags, questions, activeQuestion, setActiveQuestion }) => {
  ...
  const filteredQuestions = questions.filter(stringFilter).filter(typesFilter).filter(tagsFilter);

  return (
    <>
      <h1 className="font-oswald text-blue-400 text-4xl m-5 my-6">All Questions</h1>

      <Filter tags={availableTagsUnique} filters={filters} dispatchFilters={dispatchFilters} />
      ...
      <ul className="bg-white ml-3">
        {filteredQuestions.map(question => {
          return (
            <li
              key={question.id}
              className={classNames("p-3 border border-b-1 flex cursor-pointer group", {
                "bg-gray-200 hover:bg-gray-300": activeQuestion === question.id,
            ...
            >
              <div className="flex-grow">
                ...
                <span className="inline-block mr-5">{QuestionTypeLabels[question.type]}</span>
                ...
                <p className="inline-block mr-5">
                  ...
                  {question.tags.map(tag => (
                    <span key={tag.id} className="mr-2 text-xs border border-gray-300 rounded-full bg-gray-100 p-1 px-2 group-hover:bg-gray-300">
                      {tag.text}
                    </span>
                  ))}
                </p>
